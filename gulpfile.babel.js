...
gulp.task('bundle', ['build'], () => {
  ...
  const stream = gulp.src('./dist/index.js')
    ...
    .pipe(rename(`kinvey-backbone-sdk-${pkg.version}.js`))
    .pipe(gulp.dest(`${__dirname}/dist`))
    .pipe(rename('kinvey-backbone-sdk.min.js'))
    .pipe(buffer())
    .pipe(uglify())
    .pipe(banner(header, { pkg: pkg }))
    .pipe(gulp.dest(`${__dirname}/dist`))
    .pipe(rename(`kinvey-backbone-sdk-${pkg.version}.min.js`))
    .pipe(gulp.dest(`${__dirname}/dist`))
    .on('error', errorHandler);
  return stream;
});
...
gulp.task('upload', ['build'], () => {
  ...
  const stream = gulp.src([
    `dist/kinvey-backbone-sdk-${pkg.version}.js`,
    `dist/kinvey-backbone-sdk-${pkg.version}.min.js`
  ])
    .pipe(plumber())
    .pipe(s3({
